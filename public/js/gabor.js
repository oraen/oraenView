...
export function clearCanvas(canvas, background = 158) {
...
export function drawGabor(canvas, x, y, options) {
  const context = canvas.getContext("2d");
...
export function drawSingle(canvas, stimulus) {
...
export function drawTriple(canvas, stimulus) {
  clearCanvas(canvas, stimulus.background);
  ...
  drawGabor(canvas, cx - vector.x, cy - vector.y, { ...shared, contrast: stimulus.flankerContrast });
  if (stimulus.showTarget) drawGabor(canvas, cx, cy, { ...shared, contrast: stimulus.targetContrast });
  drawGabor(canvas, cx + vector.x, cy + vector.y, { ...shared, contrast: stimulus.flankerContrast });
}

export function drawShifted(canvas, stimulus) {
  clearCanvas(canvas, stimulus.background);
  const cx = canvas.width / 2;
  const cy = canvas.height / 2;
  const vector = layoutVector(stimulus.layoutAngle, stimulus.spacing);
